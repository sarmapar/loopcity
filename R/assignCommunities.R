...
#'  "walktrap", "leiden", "infomap", "labelprop", or "edge_between"
...
assignCommunities <- function(loops,
  ...
  relations = data.frame(from = anc$first, to = anc$second, weights = loops$score)
  relations[is.na(relations)] <- 0

  g <- igraph::graph.data.frame(relations, directed=FALSE, vertices=unique(c(anc[[2]],anc[[1]])))
  ...
  between_g <-
    switch(clusterType,
           ...
           labelprop = igraph::cluster_label_prop(g_filter,
                                          weights = relations_filter$weights),
           edge_between = igraph::cluster_edge_betweenness(g_filter,
  ...
  anchor1com <- InteractionSet::regions(loops)[InteractionSet::anchorIds(loops)$first]$anchorCommunity
  anchor2com <- InteractionSet::regions(loops)[InteractionSet::anchorIds(loops)$second]$anchorCommunity
