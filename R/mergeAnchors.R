...
mergeAnchors <- function(loops, pixelOverlap, dropDups = T){
    ...
    if(length(resolution) > 1){
        rlang::abort(c("Interaction ranges must have the same width to use pixelOverlap.",
                "x"=glue("There are {length(resolution)} different widths in `ranges(regions(loops))`."),
                "i"="Consider using bpOverlap instead."))
    }

    ## Create data table of all overlapping anchor indexes (one line per overlap)
    overlaps <- loops |>
        InteractionSet::regions() |>
        IRanges::findOverlaps(maxgap = pixelOverlap*resolution-2) |>
        data.table::as.data.table()

    ## Condense overlaps into list of all overlapping anchor indexes (one line per unique anchor)
    ...
    overlapGroups$count <- as.numeric(table(sort(as.numeric(unlist(InteractionSet::anchorIds(loops))))))

    # helper function to pick the middle value of a vector (or the largest of 2 middle vals)
    ...
    ## Replace original loop anchor IDs with new anchor IDs and create new GInteractions object
    mergedLoops <- loops
    ...
    if(dropDups){
        if(length(mergedLoops) != length(unique(mergedLoops))){
            message(glue("Duplicates dropped. {length(mergedLoops)} interactions reduced to {length(unique(mergedLoops))} interactions."))
        }
